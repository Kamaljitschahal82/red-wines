...
High levels of alcohol are asociated with low density, which makes sense, since alcohol is less dense than water, the [main component](http://www.terroir-france.com/wine/components.htm) of wine.
...
I think the analysis could be improved with more data:

* Access to the individual evaluations for every wine could give some insights about the shape of the _quality_ distribution: is the scarcity of wines of low and high quality due to calculating the median of very dispersed scores?

* In order to be sure about the trends identified, more observations of wines of extreme quality may help, specially if there were wines in the dataset with quality ratings below three and above eight, to check if those trends are still present. Maybe a quality scale from zero to ten is too fine-grained for subjective evaluations, and a shorter one could help the quality of the data.
